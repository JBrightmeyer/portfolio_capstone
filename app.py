...
def add_comment():
    ...
    if form.validate_on_submit():
        comment = Comment(name=form.name.data, content=form.comment.data)
...
def view_profile(userid):
    ...
    education_serial = [
        Education.serialize_education(school) for school in user.education
    ]
    skills_serial = [Skill.serialize_skill(skill) for skill in user.skills]
    skills_serial.sort(key=lambda x: x["description"])
    return render_template(
        "/public/user_public_profile.html",
        user=user_serial,
        jobs=jobs_serial,
...
def edit_profile(userid):
    ...
    return render_template(
        "/private/user_private_profile.html",
        user=user_serial,
        jobs=jobs_serial,
        education=education_serial,
        skills=skills_serial,
...
def get_portfolio(userid):
    ...
    return render_template(
        "/public/user_public_portfolio.html", user=user, projects=serial_projects
    )
...
def edit_portfolio(userid):
    ...
    serial_projects.sort(key=lambda x: x["title"])
    return render_template(
        "/private/user_private_portfolio.html", user=user, projects=serial_projects
    )
...
# If you'd notice i change most loops to list comprehensions, this is just a personal preference but it's generally more readable and faster than a normal for loop. You can read more about it here: https://www.pythonforbeginners.com/basics/list-comprehensions-in-python