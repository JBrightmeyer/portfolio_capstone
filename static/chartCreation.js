        ...
        .then((projects) => {
            ...
            //if there are projects returned
                projects.data.forEach(project => {
                    //queries the GitHub API to retrieve ReadMe
                    axios.get(`https://api.github.com/repos/${project["owner_name"]}/${project["project_name"]}/readme`, {headers:{"accept":"application/vnd.github.html+json"}}).then((data) =>{
                        //sets the ReadMe to the associated modal
                        document.getElementById(`${project["id"]}-readme`).innerHTML = `<div class="col">${(data.data)}</div>`})
